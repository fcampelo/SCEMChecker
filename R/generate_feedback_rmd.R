...
#
# We will save smaller R objects to disk so the Rmd can load them and display content & plots.
generate_feedback_rmd <- function(summary_df,
  ...
  for (i in seq_len(nrow(summary_df))) {
    ...
    lines <- c(lines,
               "```{r sub_vars, echo=FALSE, results='asis'}",
    ...
    lines <- c(lines,
               "```{r ref_vars, echo=FALSE, results='asis'}",
